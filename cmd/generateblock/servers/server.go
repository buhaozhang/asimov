...
type NodeServer struct {
	...
	blkTmplGenerator     *mining.BlkTmplGenerator
...
func NewServer(db database.Transactor, stateDB database.Database, agentBlacklist, agentWhitelist []string,
	...
	s.blkTmplGenerator = blockTemplateGenerator
...
func (s *NodeServer) Consensus() *satoshiplus.SPService {
	return s.consensus.(*satoshiplus.SPService)
}

func (s *NodeServer) BlockChain() *blockchain.BlockChain {
	return s.chain
}

func (s *NodeServer) SyncManager() *netsync.SyncManager {
	return s.syncManager
}

func (s *NodeServer) RoundManger() *satoshiplus.RoundManager {
	return s.roundManger.(*satoshiplus.RoundManager)
}
